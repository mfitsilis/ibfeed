...
//	m_pClient->reqHistoricalData(1,c,"20160715 00:00:00","5 D","1 min","TRADES",1,2);  //1:rth, last int=2 for seconds since epoch	
...
{	// finish=1 so it can exit the message loop
	...
	else if( id == -1 && errorCode == 1100) // if "Connectivity between IB and TWS has been lost"
						  {	disconnect(); finish=1; }
	else if( id == -1 && errorCode == 1101) // if "Connectivity between IB and TWS has been restored-data lost. must resubscribe"
						  {	disconnect(); finish=1; }
	else if( id == -1 && errorCode == 1102) // if "Connectivity between IB and TWS has been restored-data maintained."
						  {	disconnect(); finish=1; }
	else if( id == -1 && errorCode == 1300) // if "TWS socket port has been reset and this connection is being dropped. Please reconnect on the new port - <port_num>"
						  {	disconnect(); finish=1; }
